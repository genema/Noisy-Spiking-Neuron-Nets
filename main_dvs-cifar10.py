...
LastEditors: GhMa
LastEditTime: 2022-10-02 19:25:19
'''
import torch
...
parser.add_argument('--timestep', default=10, type=int, help='STATIC! =10')
...
trainloader, testloader = prepare_dvs_cifar10(args)
...
if __name__ == '__main__':
    uid = uuid.uuid4().hex
    path = os.path.join(
        './results/logs/dvs-cifar10-neuron_{}-model_{}-optim_{}-scheduler_{}-lr_{}-batch_{}-t_{}-vth_{}-tau_{}-alpha{}-sigma_{}-init_{}-TET_{}'.format(
            str(args.neuron),
            str(args.model),
            str(args.optim),
            str(args.scheduler),
