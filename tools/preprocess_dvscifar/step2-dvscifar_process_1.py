...
def events_to_frames(filename, dt):
    ...
    for i in range(10):
        ...
        # timestamp, _, _, x, y, polar, 
        events[:, 5] += 1
        events[:, 5] = events[:, 5] / 2
        frames[i, events[r1:r2, 5], events[r1:r2, 3], events[r1:r2, 4]] += 1#events[r1:r2, 0]
...
def create_npy():
    ...
    fns_train = gather_addr('dvs-cifar10', 0+1, int(train_test_portion * 1000)+1)
    fns_test = gather_addr('dvs-cifar10', int(train_test_portion * 1000)+1, 1000+1)
