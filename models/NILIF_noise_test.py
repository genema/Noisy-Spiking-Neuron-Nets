'''
Author: ----
Date: 2022-07-20 15:39:55
LastEditors: ----
LastEditTime: 2022-07-20 16:46:58
'''
import torch
...
class Neuron(nn.Module):
    ...
    def _update(self, x):
        ...
        for t in range(timestep):
            ...
            u_ = self.u
            # additive noise
            # if self.training or self.n_ensembles >= 1:
            self.u += -torch.normal(
                torch.ones_like(self.u) * self.mu, self.sigma
            )  
            # extra membrane potential perturbation, for NLIF, the idle parameter
            # alpha is used as the std of the background noise.
            self.u += -torch.normal(
                torch.ones_like(self.u) * self.mu, self.alpha
            )  
